...
import { getZipArtifact, putArtifact, deleteZipArtifact } from './helper/request-store';
 ...
 * @return {Promise}
 */
async function unzip(config: { buildId: string; token: string; }): Promise<any> {
    logger.info(`Job:unzip started. buildId: ${config.buildId}`);

    try {
        const zipFile = await getZipArtifact(config.buildId, config.token);
        ...
        await Promise.all(
            zipEntries.map(async zipEntry => {
                ...
                return putArtifact(config.buildId, config.token, fileName, file);
            })
        );
    } catch (err: any) {
    ...
    try {
        await deleteZipArtifact(config.buildId, config.token);
    } catch (err: any) {
        logger.error(err.message);
    }
